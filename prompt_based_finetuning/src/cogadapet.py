# The code is adapted from previous work (link to ADAPET), written in Python (3.8) and PyTorch (2.1)

# Improves interpretability of the ADAPET method, with a focus on reducing framing effect and availability bias resulted during prompting PLMs
...
# Define a class of logic loss driven by FOL rules
class FolModule(nn.Module):
    """docstring for FolModule"""
    def __init__(self, rule_weight_init=nn.init.xavier_uniform_):
        super(FolModule, self).__init__()
        self.logic_output = nn.Linear(1, 1)
        self.rule_weight = nn.Parameter(rule_weight_init(torch.Tensor(1, 10)), requires_grad=True) # For a maximum of 10 rules
        
    def forward(self, input_ids, list_lbl):
        # input_ids: [bs, num_lbl]
        loss = self.compute_fol_loss_for_triplets(input_ids, list_lbl)
        return loss

    # Calculate a logic loss every 3 consecutive instances in a batch
    def compute_fol_loss_for_triplets(self, input_ids, list_lbl):
        loss = 0.0
        count = 0
        bs = input_ids.shape[0]
        rule_weight = torch.sigmoid(self.rule_weight)

        for i in range (0, bs, 3): 
            rel_ab = torch.argmax(input_ids[i])
            rel_bc = torch.argmax(input_ids[i+1])
            rel_ac = torch.argmax(input_ids[i+2])

            # ONLY enforce the rule for rel type 'Eff' and 'Mech'
            # if rel_ab == list_lbl.index('effect') or rel_ab == list_lbl.index('mechanism'):  
            if rel_ab != list_lbl.index('false'):
                # rule 1, ab bc --> ac
                # rule 2, bc ac --> ab
                # rule 3, ab ac --> bc
                if rel_ab == rel_bc:
                    # use MSE for loss computation
                    rule_loss_1 = self.rule_weight[0,0] * ((input_ids[i+2, rel_ac] - \
                        torch.sigmoid(self.logic_output((input_ids[i, rel_ab] + \
                        input_ids[i+1, rel_bc] - 2).view(-1, 1)))) ** 2)
                    rule_loss_1 = rule_loss_1.squeeze()
                    loss += rule_loss_1
                    count += 1 
                
                if rel_bc == rel_ac:
                    rule_loss_2 = self.rule_weight[0,1] * ((input_ids[i, rel_ab] - \
                        torch.sigmoid(self.logic_output((input_ids[i+1, rel_bc] + \
                        input_ids[i+2, rel_ac] - 2).view(-1, 1)))) ** 2)
                    rule_loss_2 = rule_loss_2.squeeze()
                    loss += rule_loss_2
                    count += 1  

                if rel_ab == rel_ac:
                    rule_loss_3 = self.rule_weight[0,2] * ((input_ids[i+1, rel_bc] - \
                        torch.sigmoid(self.logic_output((input_ids[i, rel_ab] + \
                        input_ids[i+2, rel_ac] - 2).view(-1, 1)))) ** 2) 
                    rule_loss_3 = rule_loss_3.squeeze()
                    loss += rule_loss_3
                    count += 1 

        # print('======= logic loss ====', loss)
        if count > 0:
            loss /= count
...
class CogAdapet(nn.Module):
    """docstring for CogAdapet"""
    def __init__(self, config, tokenizer, dataset_reader, fol_module): # encoder
        super(CogAdapet, self).__init__()
        # Introduce the FOL module for logic loss computation
        self.fol = fol_module
    ...
    def forward(self, batch, do_train=True):
        ...
        :return:
        '''
        # FOL constrained decoupled label loss
        pet_disc_loss = self.get_constrained_decoupled_label_loss(batch, do_train)
    ...
    def get_fol_loss(self, input_ids, list_lbl):
        fol_loss = self.fol(input_ids,list_lbl)
        return fol_loss

    def get_single_logits_and_fol_loss(self, pet_mask_ids, mask_idx, list_lbl, gold_lbl, do_train):
        '''
        ...
        # Get loss for logical constraints
        if do_train and torch.count_nonzero(gold_lbl) == bs and torch.unique(gold_lbl).shape[0] == 1:
            fol_loss = self.get_fol_loss(lbl_prob, list_lbl)
        else:
            fol_loss = torch.tensor(0.0, dtype=torch.float)

        return lbl_prob, fol_loss

    def get_constrained_decoupled_label_loss(self, batch, do_train):
        '''
        ...
        # todo: add logic loss to datasets where the label has more than 1 token
        ...
        # Datasets where the label is 1 token
            ...
            # Get lbl logits and fol_loss to regularize training
            lbl_logits, fol_loss = self.get_single_logits_and_fol_loss(pet_mask_ids, mask_idx, list_lbl, lbl, do_train) # [bs, num_lbl]
            ...
            if do_train:
                pet_disc_loss = pet_disc_loss + fol_loss # 0.5*fol_loss, 0.5 is a hyperparameter
    ...
    def predict_helper(self, batch, pattern):
        ...
        if self.config.dataset.lower() == 'fewglue/wsc':
            ...
            bs = pet_mask_ids.shape[0]
            dummy_lbl = torch.tensor([-1]*bs, dtype=torch.long)
            lbl_logits, _ = self.get_single_logits_and_fol_loss(pet_mask_ids, mask_idx, list_lbl, dummy_lbl, do_train=False)
